...
export class SessionManager {
  private backend: BrowserBackend | null;
  private browser: Browser | null = null;
  private refreshPromises = new Map<DestinationId, Promise<DisneySession | null>>();
  private initialized = false;

  /**
   * @param backend - Optional pre-built browser backend. When omitted (the
   *   production default), getBrowser() auto-creates one from config. Tests
   *   inject a fake backend to avoid launching a real browser.
   */
  constructor(backend?: BrowserBackend) {
  ...
  private async doRefreshSession(destination: DestinationId): Promise<DisneySession | null> {
    ...
    try {
      ...
      const context = await browser.newContext({
        ...
        timezoneId: getTimezone(destination),
      });
  ...
  private async extractSession(
    ...
    const tokens = extractTokens(cookies, storageState.origins);

    // Calculate expiration
    const expiresAt = calculateExpiration(cookies);
